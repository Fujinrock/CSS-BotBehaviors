...
**NOTE:** The plugin requires a custom signature scanning library, a customized version of AlliedModder's [CDetour](https://github.com/alliedmodders/sourcemod/tree/master/public/CDetour) detouring library and some other files which are currently not publicly available. **THIS MEANS THE PLUGIN CANNOT BE BUILT OUT OF THE BOX!** As of now, the plugin is only meant to showcase the ideas behind it.
...
- Bots' hunting behavior has been modified to make them check zones (bomb or rescue zones) instead of enemy spawns or the least recently checked areas. The bots may also choose to hide instead of hunting for more varied behavior.
...
- Team flashes for bots are disabled and their flashed time has been reduced.
 ...
 - The functionality of the "nav_load" command has been extended to allow loading nav files with arbitrary names. For example, a nav file by the name "de_dust2_long.nav" can be loaded with the command "nav_load long" (.nav extension should be omitted) while de_dust2 is loaded. "nav_load de_dust2_long" would have the same result.
